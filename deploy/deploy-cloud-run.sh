...
gcloud run services update "$TASK_WORKER_SERVICE_NAME" --region="$REGION" \
  --update-env-vars="HEALER_GCP_PROJECT=$PROJECT_ID,HEALER_TASKS_LOCATION=$REGION,HEALER_TASKS_QUEUE=$TASK_QUEUE,HEALER_TASK_WORKER_URL=$TASK_WORKER_URL,HEALER_TASK_DISPATCHER_SERVICE_ACCOUNT=$DISPATCHER_SA"
